...
// 후킹 제목 3안 중 사용자가 고른 것을 저장한다 (다운로드/화면 표시에 사용).
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const db = getDb();
  db.prepare("UPDATE reels_titles SET chosen = 0 WHERE reels_job_id = ?").run(jobId);
  const result = db
    .prepare("UPDATE reels_titles SET chosen = 1 WHERE reels_job_id = ? AND candidate_index = ?")
